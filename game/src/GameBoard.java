...
public class GameBoard extends TileGrid {
    ...
    public GameBoard(ClickSwapper swapper, FileDecoder filedecoder) {
        ...
        // if tile count is not a square number it calculates the overflow and
        // accounts for it else it returns 0
        int tileoverflowY = (int) (Math.ceil(
                (filedecoder.getTileNum() - Math.pow(Gridnum, 2)) / Gridnum));
        int tileoverflowX = ((int) (filedecoder.getTileNum()
                - Math.pow(Gridnum, 2)) % Gridnum);
        ...
        for (int y = 0; y < (Gridnum + tileoverflowY); y++) {
            center.gridy = y;
            for (int x = 0; x < (Gridnum
                    + (((y + 1 >= (Gridnum + tileoverflowY))
                            && (tileoverflowY > 0) && (tileoverflowX != 0))
                                    ? (tileoverflowX - Gridnum)
                                    : 0)); x++) {
